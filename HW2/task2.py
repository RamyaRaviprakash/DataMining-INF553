...
if __name__ == "__main__":
    ...
    mapped_input_rdd = input_rdd.filter(lambda x: not str(x).startswith('"TRANSACTION_DT"')) \
        .map(lambda line: (line.split(",")[0].strip('"') + "-" + line.split(",")[1].strip('"'),
                           [line.split(",")[5].strip('"').lstrip("0")]))
    baskets_rdd = mapped_input_rdd.reduceByKey(lambda a, b: a + b).filter(lambda a: len(a[1]) > filter_threshold).map(
        lambda a: (a[0], set(a[1])))
    # baskets_rdd = baskets_rdd.partitionBy(baskets_rdd.getNumPartitions(), f)
    total_number_of_baskets = baskets_rdd.count()
    # print("total_number_of_baskets: ", total_number_of_baskets)
    # print("all baskets: ", baskets_rdd.collect())
